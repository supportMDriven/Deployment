...
if [ -n "$UPDATE_APP" ] && [ "$UPDATE_APP" != "false" ]; then
    ...
    if [ "$(uname -m)" = "x86_64" ] || [ "$(uname -m)" = "amd64" ]; then
        BASE_API="https://mdriven.net/Rest/ProductRelease/Get?vProduct=TurnkeyCore&platform=linux"
    else
        BASE_API="https://mdriven.net/Rest/ProductRelease/Get?vProduct=TurnkeyCore&platform=linux%20arm64"
    fi
